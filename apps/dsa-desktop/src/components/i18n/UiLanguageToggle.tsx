...
export const UiLanguageToggle: React.FC<UiLanguageToggleProps> = ({
  ...
  const languageCycle: Record<typeof language, typeof language> = {
    zh: 'en',
    en: 'th',
    th: 'zh',
  };
  const nextLanguage = languageCycle[language];
  const isNavVariant = variant === 'nav';
  const isRailVariant = variant === 'rail';
  const label = language === 'zh' ? t('language.uiLanguage') : t('language.current');
  const shortLabelKey =
    language === 'zh' ? 'language.short.zh' : language === 'th' ? 'language.short.th' : 'language.short.en';
  ...
  return (
    <div className={cn('relative', isRailVariant ? 'w-full' : '', wrapperClassName)}>
      ...
      >
        ...
        {isRailVariant ? (
          <span className={labelClassName}>{t(shortLabelKey)}</span>
        ) : isNavVariant ? (
